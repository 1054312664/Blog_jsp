...
public class ConsumerServlet extends HttpServlet {
	...
	public void front_updateConsumerForm(HttpServletRequest request,
		...
		if (consumerDao.front_updateConsumerForm(form)) {
			out.print("<script language=javascript>alert('修改用户成功，请重新登录！');window.location.href='dealwith.jsp?sign=2';</script>");
		} else {
			out.print("<script language=javascript>alert('修改用户信息失败！');history.go(-1);</script>");
		}
	...
	public void deleteConsumerForm(HttpServletRequest request,
		...
		if (consumerDao.deleteConsumerForm(account)) {
			out.print("<script language=javascript>alert('删除此用户成功，请重新进行查询！');window.location.href='back_consumerSelect.jsp';</script>");
		} else {
			out.print("<script language=javascript>alert('删除用户信息失败！');history.go(-1);</script>");
		}
	...
	public void queryConsumerHostForm(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		consumerDao = new ConsumerDao();
		request.setAttribute("form", consumerDao.getConsumerForm("zhou"));
		RequestDispatcher requestDispatcher = request
				.getRequestDispatcher("back_consumerSelectHostForm.jsp");
		requestDispatcher.forward(request, response);
